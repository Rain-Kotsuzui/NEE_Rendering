...
Vector3f Vector3f::max(const Vector3f &b)const
{
	return Vector3f(std::max(x(), b.x()),
					std::max(y(), b.y()),
					std::max(z(), b.z()));
}
Vector3f Vector3f::pow(const float &r) const
{
	return Vector3f(std::pow(x(), r), std::pow(y(), r), std::pow(z(), r));
}
